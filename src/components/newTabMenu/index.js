...
function NewTabmenu({ data, routineId, setCurrentWorkouts }) {
  const items = data;
  const curWorkout1 = items.filter((item) => item.category === '웨이트운동' && item.default === true);
  const curWorkout2 = items.filter((item) => item.category === '유산소운동' && item.default === true);
  const curWorkout3 = items.filter((item) => item.category === '휴식' && item.default === true);
